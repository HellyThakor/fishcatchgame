...
diamondImage.src = 'diamond.png'; // Make sure this file exists
...
// Diamond properties
const diamondWidth = 30; // Set the width of the diamond image
const diamondHeight = 30; // Set the height of the diamond image
let diamondX = Math.random() * (canvas.width - diamondWidth);
let diamondY = 0;
let diamondSpeed = 3;
...
// Draw diamond using the diamond image
function drawDiamond() {
    context.drawImage(diamondImage, diamondX, diamondY, diamondWidth, diamondHeight);
}
...
function update() {
    ...
    if (
        diamondY + diamondHeight > fishY &&
        diamondX + diamondWidth > fishX &&
...
function resetDiamond() {
    diamondX = Math.random() * (canvas.width - diamondWidth);
    diamondY = 0;
    if (score % 5 === 0) {
...
// Ensure the game starts only when both images are loaded
fishImage.onload = function() {
    diamondImage.onload = gameLoop;
};
